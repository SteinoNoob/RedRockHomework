...
import blank_avatar from "./assets/blank-avatar.svg"

// 默认封面图片
window.img = new Image();
img.src = logo;
$("#album img")[0].src = logo;

$("#logo").src = slogan;

$("#blank-avatar").src = blank_avatar;
 ...
 */
function songLoadForSearchPage (src) {
    ...
    loop: function () {
        var interval = setInterval(() => {
            if (!this.audio.paused) {
                this.bar.val(Math.round(this.audio.currentTime).toString());
            } else {
