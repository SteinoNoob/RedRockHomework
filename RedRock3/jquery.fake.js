...
(function (window, undefined) {
    ...
    var JQuery = function (se) { return new JQuery.fn.init(se); };
    JQuery.fn = JQuery.prototype = {
        /**core method of JQuery
         * @param {string|document} s a css selector @return {JQuery}*/
        init: function (s) {
            var arr = [], ret, p, blk = [], reg = /<(\w+)>/g;
            if (!s) { return this; }
			if (s.nodeType) {
			...
			if (s.match(reg)) {
				var dom = document.createElement(s.replace(reg, function($, $1){ return $1; }));
					JQuery.extend.call(dom,JQuery.prototype);
					return dom;
			}
            else {
                var eles = document.querySelectorAll(s), ret;
        ...
        prevent: function () {
            (arguments[0] === false) ? this.removeClass("click-disabled") : this.addClass("click-disabled");
            return this;
        },
        addClass: function (klass) { this.length ? this.each(function (a) { a.classList.add(klass) }) : this.classList.add(klass); return this; },
        removeClass: function (claas) { this.length ? this.each(function (a) { a.classList.remove(claas) }) : this.classList.remove(claas); return this; },
        ...
        box: function (w, h) { this.css({ width: w, height: h }); return this; },
        plus: function (m) { this.length ? this.each(function (a) { a.innerHTML += m; }) : this.innerHTML += m; return this; },
        div_middle: function () {
            var wid = this.offsetWidth,
                par_wid = this.parentElement.offsetWidth;
            this.css({
                margin_left: ((par_wid - wid) / 2).toString() + "px",
                margin_right: ((par_wid - wid) / 2).toString() + "px"
            });
            return this;
        },
        ready: function (fun) {
